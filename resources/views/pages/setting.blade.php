            ...
            <tbody>
                @foreach($setting as $val)
                    <tr>
                        ...
                        @if($val->status === '1')
                        <td><a href="{{ route('manage.setting.disable' , $val->id) }}" class="btn btn-danger">Disable</a> <a href="{{ route('manage.setting.delete' , $val->id) }}" class="btn btn-danger">Delete</a></td>
                        @endif
                        @if($val->status === '2')
                        <td><a href="{{ route('manage.setting.activate' , $val->id) }}" class="btn btn-success">Activate</a> <a href="{{ route('manage.setting.delete' , $val->id) }}" class="btn btn-danger">Delete</a></td>
                        @endif
                    </tr>
                @endforeach
            </tbody>
...
<script>
    ...
    //                         </span>
    //                     </a>
    //                     <a class="btn btn-sm btn-clean btn-icon" href="/manage/store/menu/${id}" title="Delete"> <span class="svg-icon svg-icon-md">
...
<div class="modal fade" id="myModalInsert" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="staticBackdrop" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Insert Data Setting</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <i aria-hidden="true" class="ki ki-close"></i>
                </button>
            </div>
            <div class="modal-body" id="modal-body">
                <form class="form" id="form" method="POST" action="{{ route('manage.setting.insert') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        <div class="form-group">
                            <label for="">Value</label>
                            <input type="number" name="value" class="form-control form-control-solid" required>
                        </div>
                    </div>
